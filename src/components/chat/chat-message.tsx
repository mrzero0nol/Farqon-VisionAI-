  ...
  return (
    <div className={cn('flex items-start space-x-3 py-3', isUser ? 'justify-end' : '')}>
      ...
      <div className={cn(
          ...
          isUser ? 'bg-primary text-primary-foreground' : (isError ? 'bg-destructive text-destructive-foreground' : 'bg-card text-card-foreground border'),
        )}
      >
        <p className="text-sm whitespace-pre-wrap">{message.content}</p>
        {message.image && (
          <Card className="mt-2 overflow-hidden bg-background/20 border-none">
